...
def exact_sol(X, opt):
    ...
    return np.array([ fx(X[i],opt) for i in range(len(X)) ] )


def fx(x,opt):  
    ans = -4 *math.exp(2-x)/(math.exp(x) +  4 * math.exp(x+1)-4*math.exp(2))
    return ans
...
def getMaxErr(opt): #for each grid size get max local error
    YY = []
    XX = []
    for i in range(10, 1000, 10):
        ...
        n = len(Y1)
        for j in range(0, n):
            Y1[j] = abs(Y1[j] - fx(X1[j],opt))
            Y1[j] = float("{0:.5f}".format(Y1[j]))
        YY.append(max(Y1))
        XX.append(i)
    return XX, YY
...
def draw_max_error_graph(opt):
    ...
    print(XX)
    print(YY)
    plt.plot(X, Mx_errors) #commented because it is making problems 
...
step_scale = Scale(root, orient=HORIZONTAL, length=300, from_=10, to=MAX_STEPS, resolution=1, command=options.update_num_seg)
...
label_title = Label(root, text="y' = y^2 exp(x) - 2y")
